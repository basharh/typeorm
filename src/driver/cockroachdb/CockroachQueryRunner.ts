...
export class CockroachQueryRunner extends BaseQueryRunner
    ...
    async commitTransaction(): Promise<void> {
        ...
        try {
            ...
            if (e.code === "40001") {
                ...
                await PromiseUtils.runInSequence(this.queries, q =>
                    this.query(q.query, q.parameters)
                );
    ...
    async createTable(
        ...
        table.columns
            ...
            .forEach(column => {
                upQueries.push(
                    new Query(
                        `CREATE SEQUENCE ${this.buildSequenceName(
                            table,
                            column
                        )}`
                    )
                ...
                downQueries.push(
                    new Query(
                        `DROP SEQUENCE ${this.buildSequenceName(table, column)}`
                    )
                );
            });
        ...
        if (
            ...
            if (newColumn.isUnique !== oldColumn.isUnique) {
                if (newColumn.isUnique) {
                    ...
                    downQueries.push(
                        this.dropIndexSql(table, uniqueConstraint)
                    );
                } else {
    ...
    async dropColumn(
        ...
        if (column.generationStrategy === "increment") {
            ...
            downQueries.push(
                new Query(
                    `CREATE SEQUENCE ${this.buildSequenceName(table, column)}`
                )
            );
        }
    ...
    async createUniqueConstraints(
        ...
        await PromiseUtils.runInSequence(uniqueConstraints, uniqueConstraint =>
                ...
                table.foreignKeys = tableForeignKeyConstraints.map(
                    dbForeignKey => {
                        ...
                        const schema =
                            ...
                            currentSchema
                                ? undefined
                                : dbTable["referenced_table_schema"];
    ...
    protected buildCreateColumnSql(table: Table, column: TableColumn) {
        ...
        if (column.isGenerated) {
            if (column.generationStrategy === "increment") {
                c += ` INT DEFAULT nextval('${this.buildSequenceName(
                    table,
                    column
                )}')`;
