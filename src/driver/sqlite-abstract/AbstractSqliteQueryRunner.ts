...
export abstract class AbstractSqliteQueryRunner extends BaseQueryRunner
    ...
    async renameTable(
        ...
        const down = new Query(
            `ALTER TABLE "${newTableName}" RENAME TO "${oldTable.name}"`
        );
    ...
    async dropColumns(
        ...
        columns.forEach(column => {
            ...
            table
                .findColumnUniques(column)
                .forEach(unique => table.removeUniqueConstraint(unique));
            table
                .findColumnIndices(column)
                .forEach(index => table.removeIndex(index));
            table
                .findColumnForeignKeys(column)
                .forEach(fk => table.removeForeignKey(fk));
        });
    }
    ...
    protected async loadTables(tableNames: string[]): Promise<Table[]> {
        ...
        // load tables
        const dbTables: ObjectLiteral[] = await this.query(
            `SELECT * FROM "sqlite_master" WHERE "type" = 'table' AND "name" IN (${tableNamesString})`
        );

        // load indices
        const dbIndicesDef: ObjectLiteral[] = await this.query(
            `SELECT * FROM "sqlite_master" WHERE "type" = 'index' AND "tbl_name" IN (${tableNamesString})`
        );

        // if tables were not found in the db, no need to proceed
        if (!dbTables || !dbTables.length) return [];
                        ...
                        if (indexColumns.length === 1) {
                            const column = table.columns.find(column => {
                                return !!indexColumns.find(
                                    indexColumn => indexColumn === column.name
                                );
                            });
    ...
    protected async recreateTable(
        ...
        if (migrateData) {
            ...
            upQueries.push(
                new Query(
                    `INSERT INTO "${newTable.name}"(${newColumnNames}) SELECT ${oldColumnNames} FROM "${oldTable.name}"`
                )
            );
            downQueries.push(
                new Query(
                    `INSERT INTO "${oldTable.name}"(${oldColumnNames}) SELECT ${newColumnNames} FROM "${newTable.name}"`
                )
            );
        }
        ...
        upQueries.push(
            new Query(
                `ALTER TABLE "${newTable.name}" RENAME TO "${oldTable.name}"`
            )
        );
        downQueries.push(
            new Query(
