    ...
    protected async dropOldForeignKeys(): Promise<void> {
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                const tableForeignKeysToDrop = table.foreignKeys.filter(
                    tableForeignKey => {
                        const metadataFK = metadata.foreignKeys.find(
                            metadataForeignKey =>
                                metadataForeignKey.name === tableForeignKey.name
                        );
                        return (
                            !metadataFK ||
                            (metadataFK.onDelete &&
                                metadataFK.onDelete !==
                                    tableForeignKey.onDelete) ||
                            (metadataFK.onUpdate &&
                                metadataFK.onUpdate !==
                ...
                if (tableForeignKeysToDrop.length === 0) return;
    ...
    protected async renameColumns(): Promise<void> {
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                if (metadata.columns.length !== table.columns.length) return;

                const renamedMetadataColumns = metadata.columns.filter(
                    column => {
                        return !table.columns.find(tableColumn => {
                            return (
                                ...
                                tableColumn.isUnique ===
                                    this.connection.driver.normalizeIsUnique(
                                        column
                                    )
                            );
                        });
                ...
                const renamedTableColumns = table.columns.filter(
                    tableColumn => {
    ...
    protected async dropCompositeUniqueConstraints(): Promise<void> {
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                if (compositeUniques.length === 0) return;
    ...
    protected async dropOldExclusions(): Promise<void> {
        ...
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                const oldExclusions = table.exclusions.filter(
                    tableExclusion => {
                ...
                if (oldExclusions.length === 0) return;
    ...
    protected async dropRemovedColumns(): Promise<void> {
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                const droppedTableColumns = table.columns.filter(
                    tableColumn => {
    ...
    protected async addNewColumns(): Promise<void> {
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                if (newColumnMetadatas.length === 0) return;

                // create columns in the database
                const newTableColumnOptions = this.metadataColumnsToTableColumnOptions(
                    newColumnMetadatas
                );
    ...
    protected async updatePrimaryKeys(): Promise<void> {
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                if (
                    primaryTableColumns.length !==
                        primaryMetadataColumns.length &&
        ...
        await PromiseUtils.runInSequence(
            ...
            async metadata => {
                ...
                await PromiseUtils.runInSequence(
                    changedColumns,
                    changedColumn =>
                        this.dropColumnReferencedForeignKeys(
                            metadata.tablePath,
                ...
                await PromiseUtils.runInSequence(
                    changedColumns,
                    changedColumn =>
                        this.dropColumnCompositeIndices(
                            metadata.tablePath,
                            changedColumn.databaseName
                        )
                );
                ...
                if (
                    ...
                    await PromiseUtils.runInSequence(
                        changedColumns,
                        changedColumn =>
                            this.dropColumnCompositeUniques(
                                metadata.tablePath,
                                changedColumn.databaseName
                            )
                    );
            ...
            async metadata => {
                ...
                const newKeys = metadata.foreignKeys.filter(foreignKey => {
                    return !table.foreignKeys.find(
                        dbForeignKey => dbForeignKey.name === foreignKey.name
                    );
                });
    ...
    protected async dropColumnReferencedForeignKeys(
        ...
        if (tablesWithFK.length > 0) {
            await PromiseUtils.runInSequence(tablesWithFK, tableWithFK => {
                ...
                return this.queryRunner.dropForeignKeys(
                    tableWithFK,
                    tableWithFK.foreignKeys
                );
            });
