    ...
    protected computeDiffColumns(subject: Subject): void {
        ...
        subject.metadata.columns.forEach(column => {
            ...
            if (subject.databaseEntity) {
                ...
                if (entityValue !== null) {
                    if (column.type === "date") {
                        normalizedValue = DateUtils.mixedDateToDateString(
                            entityValue
                        );
                    } else if (column.type === "time") {
                        normalizedValue = DateUtils.mixedDateToTimeString(
                            entityValue
                        );
                    } else if (
                        ...
                        normalizedValue = DateUtils.mixedDateToUtcDatetimeString(
                            entityValue
                        );
                        databaseValue = DateUtils.mixedDateToUtcDatetimeString(
                            databaseValue
                        );
                    } else if (
                        column.type === "json" ||
                        ...
                        normalizedValue = DateUtils.simpleArrayToString(
                            entityValue
                        );
                        databaseValue = DateUtils.simpleArrayToString(
                            databaseValue
                        );
                    } else if (column.type === "simple-enum") {
                        normalizedValue = DateUtils.simpleEnumToString(
                            entityValue
                        );
                        databaseValue = DateUtils.simpleEnumToString(
                            databaseValue
                        );
                    }
                }
        ...
        subject.metadata.relationsWithJoinColumns.forEach(relation => {
            ...
            if (subject.databaseEntity) {
                ...
                if (
                    ...
                    relatedEntityRelationIdMap instanceof Object
                )
                ...
                const areRelatedIdsEqual = EntityMetadata.compareIds(
                    relatedEntityRelationIdMap,
